...
| 组件 | 目标芯片 | 作用 |
| --- | --- | --- |
| [`p4-firmware`](./p4-firmware/) | ESP32-P4 | 主控程序，负责 Wi-Fi / UART 桥接、控制台命令、MAVLink 转发 |
| [`c5-firmware`](./c5-firmware/) | ESP32-C5 | 基于 `esp-hosted` 定制的 SDIO Slave 固件，为 P4 提供无线链路 |
...
`p4-firmware` 是系统中面向业务逻辑的一侧，负责初始化 Wi-Fi、创建 MAVLink 桥接器，并通过 ESP-IDF Console 暴露运行期配置能力。
...
详细说明见 [`p4-firmware/README.md`](./p4-firmware/README.md)。

## C5 固件工作流

`c5-firmware` 是基于 `esp-hosted` 定制的 SDIO Slave 固件，面向 WTP4C5-S1 一类模块，并针对无 PSRAM 环境做了适配与裁剪。
...
详细说明见 [`c5-firmware/README.md`](./c5-firmware/README.md)。
...
- `c5-firmware` 中包含大量托管组件，默认应视为上游依赖，不建议无目的地改动。
