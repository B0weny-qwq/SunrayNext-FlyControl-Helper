...
C5 的主启动入口在 `firmware-c5/main/upstream/esp_hosted_coprocessor.c`。这个入口负责启动协处理器主流程，并组织传输、能力声明和控制逻辑。

项目自己的定制边界目前很薄，主要落在：

- `firmware-c5/main/app/service/coprocessor_entry.c`
- `firmware-c5/main/config/model/product_feature_flags.h`
...
如果你一开始就扎进 `common/` 下的公共接口和大量 `upstream/*` 细节，通常会过早进入上游实现，反而看不出项目自己的定制边界。
