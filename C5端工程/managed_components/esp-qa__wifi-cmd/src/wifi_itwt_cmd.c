...
static int wifi_cmd_itwt(int argc, char **argv)
{
    ...
    if (itwt_args.setup->count + itwt_args.teardown->count + itwt_args.suspend->count != 1) {
        ESP_LOGE(APP_TAG, "Should only specify one itwt action: <setup | teardown | suspend>");
        return 1;
    }
    ...
    if (itwt_args.setup->count) {
        if (itwt_args.wakeinvlman->count) {
            if (itwt_args.wakeinvlman->ival[0] < 0 || itwt_args.wakeinvlman->ival[0] > 65535) {
                ESP_LOGE(APP_TAG, "(itwt)expect [0, 65535], wake_invl_mant: %d", itwt_args.wakeinvlman->ival[0]);
                return 1;
            }
        }
        if (itwt_args.wakeinvlexp->count) {
            if (itwt_args.wakeinvlexp->ival[0] < 0 || itwt_args.wakeinvlexp->ival[0] > 31) {
                ESP_LOGE(APP_TAG, "(itwt)expect [0, 31], wake_invl_expn: %d", itwt_args.wakeinvlexp->ival[0]);
                return 1;
            }
        }
        if (itwt_args.minwakedur->count) {
            if (itwt_args.minwakedur->ival[0] < 0 || itwt_args.minwakedur->ival[0] > 255) {
                ESP_LOGE(APP_TAG, "(itwt)expect [0, 255], min_wake_dura: %d", itwt_args.minwakedur->ival[0]);
                return 1;
            }
        }
        if (itwt_args.twtid->count) {
            if (itwt_args.twtid->ival[0] < 0 || itwt_args.twtid->ival[0] > 32767) {
                ESP_LOGE(APP_TAG, "(itwt)expect [0, 32767], twt id: %d", itwt_args.twtid->ival[0]);
                return 1;
            }
        }
        if (itwt_args.setup_timeout_time_ms->count) {
            if (itwt_args.setup_timeout_time_ms->ival[0] < 0 || itwt_args.setup_timeout_time_ms->ival[0] > 65535) {
                ESP_LOGE(APP_TAG, "(itwt)expect [0, 65535], setup timeout time: %d", itwt_args.setup_timeout_time_ms->ival[0]);
                return 1;
            }
        }
        ...
        ESP_LOGI(APP_TAG, "(itwt)setup, trigger:%d, %s, flow_id:%d, err:0x%x", setup_config.trigger, setup_config.flow_type ? "unannounce" : "announced", setup_config.flow_id, err);
    }
    if (itwt_args.teardown->count) {
        ...
        if (flow_id >= 0) {
            ...
            ESP_LOGI(APP_TAG, "(itwt)teardown, flow_id:%d, all_twt:%d, err:0x%x", flow_id, all_twt, err);
        } else {
            ESP_LOGE(APP_TAG, "(itwt)teardown, should specify an existing flow id");
        }
    }
    if (itwt_args.suspend->count) {
        ...
        if (flow_id >= 0) {
            ...
            ESP_LOGI(APP_TAG, "(itwt)suspend, flow_id:%d, all_twt:%d, suspend:%d ms, err:0x%x", flow_id, all_twt, suspend_time_ms, err);
        } else {
            ESP_LOGE(APP_TAG, "(itwt)suspend, should specify an existing flow id");
        }
    }
...
static int wifi_cmd_itwt_probe(int argc, char **argv)
{
    ...
    if (itwt_probe_args.timeout->count) {
        if (itwt_probe_args.timeout->ival[0] > 0) {
            ESP_LOGI(APP_TAG, "(itwt)send probe req, timeout:%d ms", itwt_probe_args.timeout->ival[0]);
            err = esp_wifi_sta_itwt_send_probe_req(itwt_probe_args.timeout->ival[0]);
        } else {
            ESP_LOGE(APP_TAG, "(itwt)invalid input, timeout:%d ms", itwt_probe_args.timeout->ival[0]);
        }
    }
    ESP_LOGI(APP_TAG, "err:0x%x", err);
    return err;
}
...
void wifi_cmd_register_itwt(void)
{
    /* itwt setup/teardown */
    if (!itwt_args.end) {
        itwt_args.setup = arg_int0(NULL, "setup", "<setup_cmd>", "twt setup an individual flow id with setup command 0-2");
        itwt_args.teardown = arg_lit0(NULL, "teardown", "twt teardown an individual flow id");
        itwt_args.suspend = arg_lit0(NULL, "suspend", "twt suspend an individual flow id");
