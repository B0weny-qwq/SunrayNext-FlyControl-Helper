...
void app_main(void)
{
	esp_rom_printf("C5_BOOT: entered app_main\r\n");
	ESP_EARLY_LOGI(TAG, "entered app_main");

	/* Initialize NVS */
	esp_err_t ret = nvs_flash_init();
	ESP_EARLY_LOGI(TAG, "nvs_flash_init ret=%d", ret);

	if (ret == ESP_ERR_NVS_NO_FREE_PAGES ||
	    ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
		ESP_EARLY_LOGI(TAG, "nvs needs erase, erasing");
		ESP_ERROR_CHECK(nvs_flash_erase());
		ret = nvs_flash_init();
		ESP_EARLY_LOGI(TAG, "nvs_flash_init after erase ret=%d", ret);
	}
	ESP_ERROR_CHECK( ret );

	ESP_EARLY_LOGI(TAG, "calling esp_hosted_coprocessor_init");
	esp_hosted_coprocessor_init();
	ESP_EARLY_LOGI(TAG, "esp_hosted_coprocessor_init returned");
